...
def make_text_blocks(txt):
	...
	final_blocks = []
	for b in blocks:
		new_block = unidecode(unicode(b, encoding = "utf-8"))
...
with open('/Users/grantnelson/desktop/data_science/sfdat22_project/syllable_dict.txt') as f:
	syllable_dict = pickle.load(f)
...
with open('/Users/grantnelson/desktop/data_science/sfdat22_project/frequency_dict.txt') as f:
...
with open('/Users/grantnelson/desktop/data_science/sfdat22_project/scrabble_dict.txt') as f:
...
def process_word_frame(df):
	status = accountApi.getApiTokenStatus()
	print 'Api calls remaining:', status.remainingCalls
	if status.remainingCalls > 300:
		...
		updated_status = accountApi.getApiTokenStatus()
		print 'Api calls remaining: ', updated_status.remainingCalls
		return df
	else:
		print 'Less than 300 calls remaining! Try again later.'
		return df
...
# sentence_frames = make_frames(blocks, level = 'sentence')
...
def process_sentence_frame(df):
	df['word_count'] = df.sentence.map(count_words)
	df['sentiment'] = df.sentence.map(get_sentiment)
	df['commas'] = df.sentence.map(lambda x: len(re.findall(',', x)))
	df['semicolons'] = df.sentence.map(lambda x: len(re.findall(';', x)))
	df['colons'] = df.sentence.map(lambda x: len(re.findall(':', x)))
	df['hyphens'] = df.sentence.map(lambda x: len(re.findall('-', x)))
	df['total_punct'] = df['commas'] + df['semicolons'] + df['colons']\
	+ df['hyphens']
	return df
...
def process_block_frame(df):
	df['lexical_diversity'] = df.block.map(get_lexical_diversity)
	df['grade_level'] = df.block.map(get_grade_level)
	df['difficult_words'] = df.block.map(textstat.difficult_words)
	df['pct_dialogue'] = df.block.map(get_pct_dialogue)
	return df
